﻿//константы (словари) и общие методы
string GetCardName(int cardKey) //словарь имён карт
{
    ...
    return CardNames[cardKey];
}

int RequestUserPositiveNumber(string MessageValue = "Введите число (>0):") // запрос на положительное число от пользователя (текст запроса произвольный)
{
    while (true)
    {
...
bool RequestUserBoolAnswer(string MessageValue) // запрос от пользователя ответа "Да", "Нет" на поставленный вопрос
{
    Console.WriteLine(MessageValue + "(если \"да\" нажмите \"y\", если \"нет\" любую другую клавишу");
        return Console.ReadKey().Key == ConsoleKey.Y; 
}


// Инициализация игры, методы заполнения исходных данных
(string[] playersNames, int numDecks, int[] balance) Greetings() // метод обрамлени запрос базовых параметров, именя игроков, величина банка, применяемое кол-во колод
{
    string[] playersNames = AskNames();
    int[] balance = new int[playersNames.Length - 1];
    int numDecks = RequestUserPositiveNumber("Укажите начальный баланс игроков: ");
    for (int i = 0; i < balance.Length; i++)
    {
        balance[i] = numDecks;
    }
    numDecks = RequestUserPositiveNumber("Сколько колод возьмём: "); // Возможно нужно устанавливать количество колод автоматически, в зависимости от числа игроков
...
int[] MakeBets(string[] playersNames, int[] balance, int[] bets) //опрос всех игроков о их ставке, количество игроков и их балансов должны быть массивы одинакового размера
{
    int playersCount = playersNames.Length - 1;
    for (int i = 0; i < playersCount; i++)
    {
        bets[i] = AskForBet(playersNames[i], balance[i]);
    }
    return bets;
}
...
int AskForBet(string playerName, int playerBalance) //метод опроса отдельного игрока, переспрашивает пока ставка не будет больше 0 и меньше баланса.
{
    while (true)
    {
        int betAmount = RequestUserPositiveNumber($"{playerName} у вас {playerBalance} фишек, делайте вашу ставку: ");
...
void InitGame()
{
    ...
    int[] gameDeck = Mixing(52,numDecks);
    int nextCard = 0;
    int[][] cardsOnTable = new int[playersAmount][];

    bool resumeGame = true;
    while (resumeGame)
    {
        //if nextCard > playDeck.Length * 2 / 3 then Mixing(52,numDecks);
        MakeBets(playersNames, balance, bets); //делаем ставки
        playersCardsScores = RunGame(numDecks, playersNames, gameDeck); //запускаем игру
        Scoring(balance, bets, playersCardsScores, playersNames); //подсчитываем и сообщаем резульаты раунда
        Console.WriteLine();
        resumeGame = RequestUserBoolAnswer("Следующий раунд? (напишите \"y\" если да, все что угодно другое если нет)");
    }
}

//Код игры
int[] RunGame(int numDecks, string[] playersNames, int[] playDeck)
{
    (int[,] playersDecks, int nextCard) = SetUp(playersNames, playDeck);
    return Round(playDeck, playersDecks, playersNames, nextCard);
}
